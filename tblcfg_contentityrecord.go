...
type ContentityRecord struct { // has has has Raw
	Idx_Contentity int
	...
	// Also if analysis failed, if (for example)
...
// LwDITA content type).
var ColumnSpecs_Contentity = []D.ColumnSpec{
	D.DD_RelFP,
	D.DD_AbsFP,
	D.DD_T_Cre,
	D.DD_T_Imp,
	...
	// D.ColumnSpec{D.D_TXT, "metaraw", "Meta (raw)", "Metadata/header (raw)"},
	// D.ColumnSpec{D.D_TXT, "textraw", "Text (raw)", "Text/body (raw)"},
	D.ColumnSpec{D.TEXT, "mimetype", "MIME type", "MIME type"},
	D.ColumnSpec{D.TEXT, "mtype", "MType", "MType"},
	// D.ColumnSpec{D.D_TXT, "roottag", "Root tag", "XML root tag"},
	// D.ColumnSpec{D.D_TXT, "rootatts", "Root att's", "XML root tag attributes"},
